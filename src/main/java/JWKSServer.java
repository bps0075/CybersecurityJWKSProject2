...
public class JWKSServer {
    ...
    static class JWKSHandler implements HttpHandler {
        ...
        private String buildJWKSResponse(KeyPair keyPair) {
            ...
            return String.format("{\"keys\":[{\"kty\":\"RSA\",\"kid\":\"1\",\"n\":\"%s\",\"e\":\"%s\"}]}", modulus,
                    exponent);
        }
    }
    ...
    static class AuthHandler implements HttpHandler {
        ...
        private String generateJWTWithExpiry(boolean allowExpired) {
            ...
            Instant expirationTime = allowExpired ? now.minusSeconds(3600) : now.plusSeconds(3600); // Token expires in
                                                                                                    // 1 hour or is
                                                                                                    // already expired
